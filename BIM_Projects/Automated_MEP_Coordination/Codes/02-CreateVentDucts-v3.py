...
csv_rectDucts = DATA_DIR + r"\10-SMOKE\cur-smoke-rectDucts.csv"
csv_elbows = DATA_DIR + r"\10-SMOKE\cur-smoke-elbows.csv"
csv_transitions = DATA_DIR +r"\10-SMOKE\cur-smoke-transitions.csv"
csv_risers = DATA_DIR +r"\10-SMOKE\cur-smoke-ductrisers.csv"
csv_sizeBlocks = DATA_DIR +r"P\10-SMOKE\cur-smoke-sizeBlocks.csv"
...
def create_rect_ducts_from_csv():
    ...
    with open(DATA_DIR +r'\07-VEN\CUR-RECTS.csv', 'r') as ducts_file:
...
def create_round_ducts_from_csv():
    ...
    # Read the CSV line files and create simple pipes
    with open(DATA_DIR +r'\07-VEN\CUR-ROUNDS.csv', 'r') as ducts_file:
        ducts_reader = csv.DictReader(ducts_file)    
        #get elevation of the level
        level = doc.GetElement(level_id)
        elevation = level.Elevation + 4250 / 304.8
...
def create_rect_vertducts_from_csv():
    ...
    # Read the CSV line files and create simple pipes
    with open(DATA_DIR +r'P\07-VEN\CUR-DOWNRECTS.csv', 'r') as ducts_file:
        ducts_reader = csv.DictReader(ducts_file)    
        #get elevation of the level
        level = doc.GetElement(level_id)
